...
const requestPairingCode = async (phoneNumber, pairKey) => {
  ...
  if (pairKey) {
    const code = pairKey.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 8);
    authState.creds.pairingCode = code;
  } else {
    authState.creds.pairingCode = (0, crypto_1.randomBytes)(5).toString("hex").slice(0, 8).toUpperCase();
  }
    authState.creds.me = {
