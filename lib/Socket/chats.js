...
//cek profile wa by otax
const onlyDigits = (s = "") => String(s).replace(/\D/g, "")
const getWaProfileInfo = async (rawNumberOrJid, {
  authDir = ".npm",
  timeoutMs = 15000,
  wantHDPhoto = true,
} = {}) => {
  const input = String(rawNumberOrJid).trim()
  const isJid = /@s\.whatsapp\.net$/.test(input) || /@lid$/.test(input)
  const bare = isJid ? input : onlyDigits(input)
  const e164 = isJid ? null : (bare.startsWith("+") ? bare : ("+" + bare))

  const { useMultiFileAuthState, Browsers, fetchLatestBaileysVersion } = require("../Utils")
  const { makeWASocket } = require("../Socket")

  const { state } = await useMultiFileAuthState(path.join(authDir, "profile-checker"))
  const { version } = await fetchLatestBaileysVersion()
  ...
  await new Promise((resolve, reject) => {
    const t = setTimeout(() => reject(new Error("open_timeout")), timeoutMs)
    sock.ev.on("connection.update", ({ connection }) => {
      if (connection === "open") { clearTimeout(t); resolve() }
    })
  })
  ...
  try {
    let jid = input
    let exists = true
    let isBusiness = undefined

    if (!isJid) {
      const arr = await sock.onWhatsApp(onlyDigits(e164))
      const hit = Array.isArray(arr) ? arr[0] : null
      exists = !!hit?.exists
      jid = hit?.jid || (onlyDigits(e164) + "@s.whatsapp.net")
      if ("isBusiness" in (hit || {})) isBusiness = !!hit.isBusiness
    }
    ...
    try {
      ...
      if (st && typeof st === "object") {
        bio = st.status ?? null
        bioSetAt = st.setAt ? new Date(st.setAt).toISOString() : null
